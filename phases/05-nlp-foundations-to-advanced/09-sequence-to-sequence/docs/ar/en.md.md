...
**Teacher forcing.** During training, the decoder's input at step `t` is the _ground-truth_ token at position `t-1`, not the decoder's own previous prediction. This stabilizes training; without it, early mistakes cascade and the model never learns. At inference, you have to use the model's own predictions, so there is always a train/inference distribution gap. That gap is called **exposure bias**.
...
Attention (lesson 10) fixes this by letting the decoder look at _every_ encoder hidden state, not just the last one. That is the whole pitch.
