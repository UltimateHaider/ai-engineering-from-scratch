...
**One Bahdanau / Luong gotcha worth naming.** Bahdanau uses `s_{t-1}` (the decoder state _before_ generating the current word). Luong uses `s_t` (the state _after_). Mixing them up produces subtly wrong gradients that are extremely hard to debug. Pick one paper and stick to its convention.
