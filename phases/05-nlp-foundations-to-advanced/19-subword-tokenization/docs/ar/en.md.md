...
**SentencePiece vs tiktoken.** SentencePiece is the library that _trains_ vocabularies (BPE or Unigram) directly on raw Unicode text, encoding whitespace as `▁`. tiktoken is OpenAI's fast _encoder_ against pre-built vocabularies; it does not train.
