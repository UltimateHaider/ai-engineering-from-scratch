...
Original paper: `x + sublayer(LN(x))` vs `LN(x + sublayer(x))`. Post-norm lost favor around 2019 — it is harder to train deeply without careful warmup. Pre-norm (`LN` _before_ sublayer) is the 2026 default: Llama, Qwen, GPT-3+, Mistral all use it.
