...
Crucially, the encoder runs once per input. The decoder runs autoregressively but cross-attends to the _same_ encoder output at every step. Caching the encoder output is a free speedup for long inputs.
