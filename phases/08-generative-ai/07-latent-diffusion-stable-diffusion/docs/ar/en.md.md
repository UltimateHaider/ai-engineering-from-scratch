...
Most of those FLOPs go to pushing perceptually unimportant details through the net — the high-frequency texture that a lossy VAE could compress away. Rombach's idea: train a VAE once (the _first stage_), freeze it, and run diffusion entirely in the 4-channel 64×64 latent space (the _second stage_). Same U-Net. 1/16th the pixels. ~64x fewer FLOPs for comparable quality.
...
1. **Staggered loading.** Flux has three networks that never need to coexist in VRAM: T5-XXL text encoder (~10 GB in fp32), CLIP-L (small), the 12B MMDiT, and the VAE. Encode the prompt first, _delete_ the encoders, load the DiT, denoise, _delete_ the DiT, load the VAE, decode. Consumer 8GB GPUs only fit one stage at a time.
