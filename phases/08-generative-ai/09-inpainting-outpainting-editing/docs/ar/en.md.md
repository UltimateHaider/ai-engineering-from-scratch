...
A client sends a perfect product photo with a distracting sign in the background. You want to erase the sign and leave everything else pixel-identical. You cannot run text-to-image from scratch — the result will have a different color, different lighting, different product angle. You want to regenerate _only_ the masked region, and you want the regeneration to respect the surrounding context.
...
- **Prompt mismatch.** The prompt should describe the _whole_ image, not just the new content. "A cat sitting on a chair" not "a cat".
