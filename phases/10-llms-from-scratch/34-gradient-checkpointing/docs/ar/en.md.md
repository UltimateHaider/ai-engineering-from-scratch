...
Split the network into `N` segments. During forward, store only the _input_ to each segment. When backward needs intermediates, rerun the segment's forward pass to materialize them, then differentiate.
...
Not all activations cost the same. The attention softmax output is `B*L*L*heads` and grows _quadratically_ with sequence length. The FFN hidden activation is `B*L*4d` and grows linearly. For long sequences the softmax dominates.
