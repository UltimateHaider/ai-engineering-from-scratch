...
**The invariant.** All three cache prefixes only. If any token differs between requests, everything after the first differing token is a miss. Put the _stable_ parts at the top, the _variable_ parts at the bottom.
...
Combine with semantic caching (Phase 11 · 11) for the user-message layer: prompt caching handles _token-identical_ reuse, semantic caching handles _meaning-identical_ reuse.
