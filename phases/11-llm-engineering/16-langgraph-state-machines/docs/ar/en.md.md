...
1. **State.** A typed dict (TypedDict or Pydantic model) that flows through the graph. Every node receives the full state and returns a partial update, which LangGraph merges using a _reducer_ per field — `operator.add` for lists that should accumulate, overwrite by default.
...
Refuse to ship a LangGraph agent that has no checkpointer. Refuse to ship one that interrupts _after_ the side effect. Refuse to ship a `messages` field without `add_messages` as its reducer.
