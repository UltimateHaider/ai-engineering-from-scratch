...
The fix is not more agents — it is _different_ agents. Assign distinct roles. Give the critic tools the planner does not have. Give the verifier an objective test suite. Now the system has internal disagreement with grounded correction, not just parallel guessing.
...
Each role has a strict input/output schema. The role prompt says what the role _is_ and what it _must produce_. The `Code = SOP(Team)` formulation — deterministic SOPs turn a team of LLMs into a predictable pipeline.
