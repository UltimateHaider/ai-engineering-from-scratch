...
5. **Governance.** Agentic DAOs: governance structures where humans _and_ agents vote on protocol changes, with voting power tied to reputation.
...
Google Research ("Mechanism design for large language models") proposes second-price token auctions for aggregating LLM outputs. Setup: N agents each propose a completion; each has a private value for being selected. The auctioneer picks the highest-value proposal and pays the _second-highest_ value. Under monotone aggregation (value depends on which proposal is chosen, not how many were bid), this is truthful — agents bid their true value.
...
2. Implement Shapley _sampling_ (Monte Carlo over K orderings). How does K affect approximation accuracy? Compare to exact for N=4.
